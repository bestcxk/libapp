    ...
    public class RRfid : IRRfid
    {
        ...
        public MessageModel<List<ScanDataModel>> NewScan(Int64 startBlockNum = -1, Int64 readBlockCount = -1)
        {
            ...
            foreach (var uid in uids)
            {
                ...
                byte errorCode = 0;
                state = 0;
                fCmdRet = ReadMultipleBlock(ref readerAddr, ref state, uid, (byte)startBlockNum, (byte)readBlockCount,
        ...
        public MessageModel<string> WriteLabel(string uidHex, byte[] data, Int64 actionBlockSize = -1, Int64 writeState = -1)
        {
            ...
            for (byte i = 0; i < writeBlockCount; i++)
            {
                ...
                fCmdRet = WriteSingleBlock(ref readerAddr, ref state, uid, i, data[startIndex..endIndex], ref errorCode, portIndex);
                if (fCmdRet != OK)
                {
                    res.msg = "写入失败";
                    return res;
